...
with tab1:
    ...
    status_options = ['Won', 'Draft', 'To be approved', 'Lost',
                      'Not lost for AM', 'Wonderful', 'Revised', 'Offered', 'Offerable']
    item_type_options = ['W', 'WI', 'S', 'Others', 'PL', 'IPL', 'SLAWR']
    country_options = [28., 25., 30., 32., 38., 78., 27.,
                       77., 113., 79., 26., 39., 40., 84., 80., 107., 89.]
    ...
    with st.form("my_form"):
        ...
        with col1:
            ...
            application = st.selectbox(
                "Application", sorted(application_options), key=4)
        ...
        with col3:
            ...
            quantity_tons = st.text_input(
                "Enter Quantity Tons (Min:611728 & Max:1722207579)")
    ...
    if submit_button and flag == 0:
        ...
        new_sample = np.array([[np.log(float(quantity_tons)), application, np.log(float(
            thickness)), float(width), country, float(customer), int(product_ref), item_type, status]])
        new_sample_ohe = t_loaded.transform(new_sample[:, [7]]).toarray()
        new_sample_be = s_loaded.transform(new_sample[:, [8]]).toarray()
        new_sample = np.concatenate(
            (new_sample[:, [0, 1, 2, 3, 4, 5, 6,]], new_sample_ohe, new_sample_be), axis=1)
...
with tab2:
    ...
    with st.form("my_form1"):
        ...
        with col3:
            ...
            capplication = st.selectbox(
                "Application", sorted(application_options), key=41)
    ...
    if csubmit_button and cflag == 0:
        ...
        new_sample = np.array([[np.log(float(cquantity_tons)), np.log(float(cselling)), capplication, np.log(
            float(cthickness)), float(cwidth), ccountry, int(ccustomer), int(product_ref), citem_type]])
        new_sample_ohe = ct_loaded.transform(new_sample[:, [8]]).toarray()
        new_sample = np.concatenate(
            (new_sample[:, [0, 1, 2, 3, 4, 5, 6, 7]], new_sample_ohe), axis=1)
...
st.write(f'<h6 style="color:rgb(0, 153, 153,0.35);">App Created by Balaji Chakrapani</h6>',
         unsafe_allow_html=True)
